...
public class DependencyTree {
    ...
    public void listDependencyPackages(BLangPackage packageNode) {
        outStream.println(printPackageInfo(packageNode.packageID));
    ...
    public static List<StringBuilder> renderDependencyTreeLines(BLangPackage packageNode, SymbolTable symbolTable,
        ...
        if (depth > 0) {
            result.add(new StringBuilder().append(printPackageInfo(packageNode.packageID)));
        }
     ...
     * @return string of packageID
     */
    public static String printPackageInfo(PackageID packageID) {
        String orgName = "";
        if (packageID.orgName != null && packageID.orgName != Names.ANON_ORG) {
            orgName = packageID.orgName + "/";
        }

        if (packageID.version == Names.DEFAULT_VERSION || packageID.version == Names.EMPTY) {
